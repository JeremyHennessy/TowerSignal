...
export function AuthLandingPage({
  ...
  return <main className="auth-gate-page">
    <section className="auth-brand-panel" aria-label="TowerSignal product introduction">
      <div className="auth-brand-lockup"><span className="auth-brand-mark">TS</span><strong>TowerSignal</strong></div>
      <div className="auth-brand-copy">
        <span className="page-kicker">Commercial intelligence for water-service markets</span>
        <h1>Know which accounts matter, why now, and what changed.</h1>
        <p>TowerSignal combines cooling-tower account intelligence, procurement activity, company relationships and workflow state in one authenticated workspace.</p>
      </div>
      <div className="auth-value-grid">
        <article><strong>Account timing</strong><span>Source-backed operational and regulatory changes.</span></article>
        <article><strong>Procurement intelligence</strong><span>Observed solicitations, awards, contracts and vendors.</span></article>
        <article><strong>Company intelligence</strong><span>Conservative vendor identity and public customer relationships.</span></article>
        <article><strong>Private workflow</strong><span>Watchlists, notes, next actions and saved views tied to your login.</span></article>
      </div>
    </section>
    ...
    <section className="auth-form-panel" aria-label="TowerSignal authentication">
      <div className="auth-form-card">
        <div className="auth-form-heading">
          ...
          <p>{mode === 'sign-up' ? 'Create a private workspace login to access the application.' : 'All TowerSignal application pages require an authenticated session.'}</p>
        </div>
