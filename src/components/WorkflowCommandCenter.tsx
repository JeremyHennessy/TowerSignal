...
import { WorkflowAccountSection } from './WorkflowAccountSection'
...
type BulkMembershipAction = 'add' | 'remove'

type WorkflowCommandCenterProps<T extends SystemSummary> = {
...
export function WorkflowCommandCenter<T extends SystemSummary>({
  user,
  systems,
  marketCount,
  accounts,
  ...
  const [bulkStatus, setBulkStatus] = useState<AccountDisposition | ''>('')
  const [bulkWatchlist, setBulkWatchlist] = useState('')
  const [bulkMembershipAction, setBulkMembershipAction] = useState<BulkMembershipAction>('add')
  ...
  const filteredEvents = useMemo(() => recentEvents
    .filter(event => filteredIds.has(event.system_id))
    .sort((a, b) => b.detected_at.localeCompare(a.detected_at)), [filteredIds, recentEvents])
  ...
  const actionRows = useMemo(() => filteredRows
    .filter(row => {
      const account = accountById.get(row.system_id)
  ...
  const selectedMembershipIds = selectedRow ? new Set(membershipIdsBySystem.get(selectedRow.system_id) ?? []) : new Set<string>()
  const selectedWatchlists = [...selectedMembershipIds].map(id => watchlistById.get(id)).filter((name): name is string => Boolean(name))
  ...
  }, [query, statusFilter, watchlistFilter, boroughFilter, attentionFilter])
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => setBulkMessage(null), 4000)
  ...
  const applyQuickView = (nextTab: WorkspaceTab, filter: AttentionFilter) => {
    setTab(nextTab)
    setAttentionFilter(filter)
  }

  const toggleChecked = (systemId: string, checked: boolean) => {
  ...
  const visibleIds = visibleRows.map(row => row.system_id)
  const visibleChecked = visibleIds.length > 0 && visibleIds.every(id => checkedIds.has(id))
  ...
  const applyBulkStatus = async () => {
    ...
    try {
      for (const systemId of checkedIds) {
        const account = accountById.get(systemId)
        await onSaveAccount(systemId, {
          status: bulkStatus,
          note: account?.note ?? '',
          next_action_date: account?.next_action_date ?? null,
        })
      }
      setBulkMessage(`Updated status for ${checkedIds.size} account${checkedIds.size === 1 ? '' : 's'}.`)
      setBulkStatus('')
    } finally {
      setBulkBusy(false)
    }
  ...
  const applyBulkWatchlist = async () => {
    if (!bulkWatchlist || checkedIds.size === 0) return
    setBulkBusy(true)
    setBulkMessage(null)
    try {
      const enabled = bulkMembershipAction === 'add'
      for (const systemId of checkedIds) await onToggleMembership(systemId, bulkWatchlist, enabled)
      setBulkMessage(`${enabled ? 'Added' : 'Removed'} ${checkedIds.size} account${checkedIds.size === 1 ? '' : 's'} ${enabled ? 'to' : 'from'} the watchlist.`)
    } finally {
      setBulkBusy(false)
    }
  }
  ...
  return <section className="workflow-command-center" aria-label="Workflow command center">
    ...
    <div className="workflow-pipeline-strip" aria-label="Workflow status distribution">
      <span>Pipeline</span>
      <button className={statusFilter === 'none' ? 'active' : ''} onClick={() => setStatusFilter(current => current === 'none' ? 'all' : 'none')}>No status <strong>{statusCounts.get('none') ?? 0}</strong></button>
      {statuses.map(status => <button key={status.value} className={statusFilter === status.value ? 'active' : ''} onClick={() => setStatusFilter(current => current === status.value ? 'all' : status.value)}>{status.label} <strong>{statusCounts.get(status.value) ?? 0}</strong></button>)}
    </div>

    <div className="workflow-command-tabs" role="tablist" aria-label="Workflow views">
      {(['accounts', 'changes', 'actions'] as WorkspaceTab[]).map(value => <button key={value} role="tab" aria-selected={tab === value} className={tab === value ? 'active' : ''} onClick={() => setTab(value)}><span>{value === 'accounts' ? 'Accounts' : value === 'changes' ? 'Changes' : 'Actions'}</span><strong>{tabCounts[value].toLocaleString()}</strong></button>)}
      <div className="workflow-command-view-summary">{checkedIds.size > 0 ? `${checkedIds.size} selected` : `${filteredRows.length.toLocaleString()} matching accounts`}</div>
    </div>

    <div className="workflow-command-filters">
      <label className="workflow-command-search">Search<input aria-label="Search workflow accounts" value={query} onChange={event => setQuery(event.target.value)} placeholder="Address, owner, borough, ID, note or watchlist" /></label>
      ...
      <label>Watchlist<select aria-label="Workflow watchlist filter" value={watchlistFilter} onChange={event => setWatchlistFilter(event.target.value)}><option value="all">All watchlists</option>{watchlists.map(watchlist => <option key={watchlist.id} value={watchlist.id}>{watchlist.name} ({watchlistCounts.get(watchlist.id) ?? 0})</option>)}</select></label>
      <label>Borough<select aria-label="Workflow borough filter" value={boroughFilter} onChange={event => setBoroughFilter(event.target.value)}><option value="all">All boroughs</option>{boroughs.map(borough => <option key={borough} value={borough}>{borough}</option>)}</select></label>
      <label>Attention<select aria-label="Workflow attention filter" value={attentionFilter} onChange={event => setAttentionFilter(event.target.value as AttentionFilter)}><option value="all">All accounts</option><option value="due">Due / overdue</option><option value="changed">Changed · 7d</option><option value="high">High priority · 70+</option><option value="contact">Contact-ready</option><option value="unscheduled">Needs next-action date</option></select></label>
      <button type="button" className="workflow-command-clear" onClick={clearFilters}>Clear</button>
    </div>
    ...
    {checkedIds.size > 0 && <div className="workflow-bulk-bar" aria-label="Bulk workflow actions">
      <strong>{checkedIds.size} selected</strong>
      <div><select aria-label="Bulk status" value={bulkStatus} onChange={event => setBulkStatus(event.target.value as AccountDisposition | '')}><option value="">Set status…</option>{statuses.map(status => <option key={status.value} value={status.value}>{status.label}</option>)}</select><button disabled={!bulkStatus || bulkBusy || busy} onClick={() => void applyBulkStatus()}>Apply status</button></div>
      <div><select aria-label="Bulk watchlist action" value={bulkMembershipAction} onChange={event => setBulkMembershipAction(event.target.value as BulkMembershipAction)}><option value="add">Add to</option><option value="remove">Remove from</option></select><select aria-label="Bulk watchlist" value={bulkWatchlist} onChange={event => setBulkWatchlist(event.target.value)}><option value="">Watchlist…</option>{watchlists.map(watchlist => <option key={watchlist.id} value={watchlist.id}>{watchlist.name}</option>)}</select><button disabled={!bulkWatchlist || bulkBusy || busy} onClick={() => void applyBulkWatchlist()}>Apply</button></div>
      <button className="workflow-bulk-clear" onClick={() => setCheckedIds(new Set())}>Clear selection</button>
      {bulkMessage && <span className="workflow-bulk-message">{bulkMessage}</span>}
    </div>}

    <div className="workflow-command-grid">
      <div className="workflow-command-primary">
        {tab === 'accounts' && <>
          <div className="workflow-account-view-toggle" aria-label="Account view mode"><span>Account view</span>{(['split', 'table', 'map'] as AccountView[]).map(value => <button key={value} className={accountView === value ? 'active' : ''} onClick={() => setAccountView(value)}>{value === 'split' ? 'Map + table' : value === 'table' ? 'Table' : 'Map'}</button>)}</div>
          {filteredRows.length === 0 ? <div className="reference-empty-state compact"><strong>No workflow accounts match these filters.</strong><span>Clear or widen the filters to return accounts to the workspace.</span></div> : <>
            {accountView !== 'table' && <div className="workflow-command-map"><TowerMap systems={filteredRows} selectedId={selectedId} onSelect={row => setSelectedId(row.system_id)} /></div>}
            {accountView !== 'map' && <div className="table-card workflow-command-table-card"><div className="table-heading"><div><strong>{filteredRows.length.toLocaleString()}</strong> workflow accounts</div><span>Showing {visibleRows.length ? activePage * PAGE_SIZE + 1 : 0}–{Math.min((activePage + 1) * PAGE_SIZE, filteredRows.length)}</span></div><div className="table-scroll"><table className="workflow-command-table"><thead><tr><th className="workflow-select-cell"><input aria-label="Select visible accounts" type="checkbox" checked={visibleChecked} onChange={event => toggleVisible(event.target.checked)} /></th><th><button onClick={() => changeSort('address')}>Account{sortIndicator('address')}</button></th><th><button onClick={() => changeSort('status')}>Status{sortIndicator('status')}</button></th><th><button onClick={() => changeSort('next_action')}>Next action{sortIndicator('next_action')}</button></th><th><button onClick={() => changeSort('priority')}>Priority{sortIndicator('priority')}</button></th><th>Why now</th><th><button onClick={() => changeSort('changes')}>Changes{sortIndicator('changes')}</button></th><th>Contact</th><th>Watchlists</th><th aria-label="Open account" /></tr></thead><tbody>{visibleRows.map(row => {
              const account = accountById.get(row.system_id)
              ...
              const state = dueState(account, today)
              return <tr key={row.system_id} className={selectedId === row.system_id ? 'selected-row' : ''} onClick={() => setSelectedId(row.system_id)} tabIndex={0} onKeyDown={event => { if (event.key === 'Enter') setSelectedId(row.system_id) }}>
                <td className="workflow-select-cell" onClick={event => event.stopPropagation()}><input aria-label={`Select ${row.address ?? row.system_id}`} type="checkbox" checked={checkedIds.has(row.system_id)} onChange={event => toggleChecked(row.system_id, event.target.checked)} /></td>
                <td className="account-cell"><strong>{row.address ?? 'Address unavailable'}</strong><span>{[row.borough, row.zip].filter(Boolean).join(' · ') || 'Location unavailable'}</span><small className="mono">{row.system_id}</small></td>
                <td><span className="status-chip">{statusLabel(account?.status)}</span></td>
                <td>{account?.next_action_date ? <><strong className={state === 'overdue' || state === 'today' ? 'due-date' : ''}>{formatDate(account.next_action_date)}</strong><small>{state === 'overdue' ? 'Overdue' : state === 'today' ? 'Due today' : 'Scheduled'}</small></> : account && ACTION_STATUSES.has(account.status) ? <span className="workflow-needs-date">Needs date</span> : <span className="muted-copy">—</span>}</td>
                ...
                <td><span className={`signal signal-${row.primary_signal.toLowerCase()}`}>{signalLabel(row.primary_signal)}</span>{row.recent_confirmed_violation && <small className="urgent-copy">Recent violation</small>}</td>
                <td>{events.length ? <strong>{events.length}</strong> : <span className="muted-copy">0</span>}</td>
                <td>{(row.hpd_contact_count ?? 0) > 0 ? <span className="contact-ready">✓ {row.hpd_contact_count}</span> : <span className="muted-copy">—</span>}</td>
                <td>{names.length ? <div className="workflow-mini-tags">{names.slice(0, 2).map(name => <span key={name}>{name}</span>)}{names.length > 2 && <span>+{names.length - 2}</span>}</div> : <span className="muted-copy">—</span>}</td>
            ...
            })}</tbody></table></div><div className="pagination"><button disabled={activePage === 0} onClick={() => setPage(current => Math.max(0, current - 1))}>Previous</button><span>Page {activePage + 1} of {maxPage + 1}</span><button disabled={activePage === maxPage} onClick={() => setPage(current => Math.min(maxPage, current + 1))}>Next</button></div></div>}
          </>}
        </>}

        {tab === 'changes' && <div className="table-card workflow-command-table-card"><div className="table-heading"><div><strong>{filteredEvents.length.toLocaleString()}</strong> source changes in the current 7-day monitor window</div><span>{counts.changed.toLocaleString()} affected workflow accounts</span></div>{filteredEvents.length === 0 ? <div className="empty-state"><strong>No recent source changes match these filters.</strong><span>Widen the account filters or return to all workflow accounts.</span></div> : <div className="table-scroll"><table className="workflow-command-table workflow-change-table"><thead><tr><th>Detected</th><th>Account</th><th>Change</th><th>Source observation</th><th>Priority</th><th>Workflow</th><th>Source</th><th aria-label="Open account" /></tr></thead><tbody>{visibleEvents.map((event, index) => {
          const row = rowById.get(event.system_id)
          const account = accountById.get(event.system_id)
          return <tr key={`${event.system_id}-${event.event_type}-${event.detected_at}-${index}`} className={selectedId === event.system_id ? 'selected-row' : ''} onClick={() => setSelectedId(event.system_id)}>
            <td><strong>{formatDate(event.detected_at)}</strong></td>
            <td className="account-cell"><strong>{row?.address ?? event.address ?? event.system_id}</strong><span>{row?.borough ?? event.borough ?? '—'}</span><small className="mono">{event.system_id}</small></td>
            <td><strong>{titleCase(event.event_type)}</strong><small>{event.evidence_confidence ?? 'Evidence confidence unavailable'}</small></td>
            <td>{event.source_observation_date ? formatDate(event.source_observation_date) : '—'}</td>
            <td>{row ? <span className={row.priority_score >= 70 ? 'priority-text-high' : ''}>P{row.priority_score}</span> : event.priority_score != null ? `P${event.priority_score}` : '—'}</td>
            <td><span className="status-chip">{statusLabel(account?.status)}</span></td>
            <td><span className="workflow-source-label">{event.source}</span></td>
            <td className="row-arrow">{row && <button className="table-link" onClick={click => { click.stopPropagation(); onOpenAccount(row) }}>›</button>}</td>
          </tr>
        })}</tbody></table></div>}<div className="pagination"><button disabled={activeEventPage === 0} onClick={() => setEventPage(current => Math.max(0, current - 1))}>Previous</button><span>Page {activeEventPage + 1} of {eventMaxPage + 1}</span><button disabled={activeEventPage === eventMaxPage} onClick={() => setEventPage(current => Math.min(eventMaxPage, current + 1))}>Next</button></div></div>}

        {tab === 'actions' && <div className="table-card workflow-command-table-card"><div className="table-heading"><div><strong>{actionRows.length.toLocaleString()}</strong> accounts requiring or carrying a next action</div><span>Overdue first · then due today · scheduled · active status without a date</span></div>{actionRows.length === 0 ? <div className="empty-state"><strong>No action items match these filters.</strong><span>Set a workflow status or next-action date from the account inspector.</span></div> : <div className="table-scroll"><table className="workflow-command-table workflow-actions-table"><thead><tr><th>Next action</th><th>Account</th><th>Status</th><th>Priority</th><th>Changes · 7d</th><th>Private note</th><th>Watchlists</th><th aria-label="Open account" /></tr></thead><tbody>{visibleActions.map(row => {
          const account = accountById.get(row.system_id)
          const state = dueState(account, today)
          const names = (membershipIdsBySystem.get(row.system_id) ?? []).map(id => watchlistById.get(id)).filter((name): name is string => Boolean(name))
          return <tr key={row.system_id} className={selectedId === row.system_id ? 'selected-row' : ''} onClick={() => setSelectedId(row.system_id)}>
            <td>{account?.next_action_date ? <><strong className={state === 'overdue' || state === 'today' ? 'due-date' : ''}>{formatDate(account.next_action_date)}</strong><small>{state === 'overdue' ? 'Overdue' : state === 'today' ? 'Due today' : 'Scheduled'}</small></> : <span className="workflow-needs-date">Needs date</span>}</td>
            <td className="account-cell"><strong>{row.address ?? row.system_id}</strong><span>{[row.borough, row.zip].filter(Boolean).join(' · ') || '—'}</span></td>
            <td><span className="status-chip">{statusLabel(account?.status)}</span></td>
            <td><span className={row.priority_score >= 70 ? 'priority-text-high' : ''}>P{row.priority_score}</span></td>
            <td>{eventsBySystem.get(row.system_id)?.length ?? 0}</td>
            <td className="workflow-action-note">{account?.note || <span className="muted-copy">No note</span>}</td>
            <td>{names.length ? <div className="workflow-mini-tags">{names.slice(0, 2).map(name => <span key={name}>{name}</span>)}{names.length > 2 && <span>+{names.length - 2}</span>}</div> : <span className="muted-copy">—</span>}</td>
            <td className="row-arrow"><button className="table-link" onClick={event => { event.stopPropagation(); onOpenAccount(row) }}>›</button></td>
          </tr>
        })}</tbody></table></div>}<div className="pagination"><button disabled={activeActionPage === 0} onClick={() => setActionPage(current => Math.max(0, current - 1))}>Previous</button><span>Page {activeActionPage + 1} of {actionMaxPage + 1}</span><button disabled={activeActionPage === actionMaxPage} onClick={() => setActionPage(current => Math.min(actionMaxPage, current + 1))}>Next</button></div></div>}
      </div>

      <aside className="workflow-account-inspector" aria-live="polite">
        {selectedRow ? <>
          <div className="workflow-inspector-head"><div><span className="page-kicker">Selected account</span><strong>{selectedRow.address ?? selectedRow.system_id}</strong><small>{[selectedRow.borough, selectedRow.zip].filter(Boolean).join(' · ') || selectedRow.system_id}</small></div><span className={`workflow-inspector-score ${selectedRow.priority_score >= 70 ? 'high' : ''}`}>P{selectedRow.priority_score}</span></div>
          <div className="workflow-inspector-signals"><span className={`signal signal-${selectedRow.primary_signal.toLowerCase()}`}>{signalLabel(selectedRow.primary_signal)}</span>{dueState(selectedAccount, today) === 'overdue' && <span className="workflow-alert-chip">Overdue</span>}{dueState(selectedAccount, today) === 'today' && <span className="workflow-alert-chip">Due today</span>}{selectedEvents.length > 0 && <span>{selectedEvents.length} change{selectedEvents.length === 1 ? '' : 's'} · 7d</span>}{selectedRow.recent_confirmed_violation && <span className="workflow-alert-chip">Recent violation</span>}</div>
          <dl className="workflow-inspector-facts"><div><dt>Equipment</dt><dd>{selectedRow.active_equipment} active</dd></div><div><dt>Latest sample</dt><dd>{selectedRow.latest_sample_date ? formatDate(selectedRow.latest_sample_date) : 'No public date'}</dd></div><div><dt>HPD contacts</dt><dd>{selectedRow.hpd_contact_count ?? 0}</dd></div><div><dt>Recent DOB</dt><dd>{selectedRow.dob_recent_activity_count ?? 0}</dd></div><div><dt>Watchlists</dt><dd>{selectedWatchlists.length ? selectedWatchlists.join(', ') : 'None'}</dd></div></dl>
          <WorkflowAccountSection signedIn={Boolean(user)} account={selectedAccount} watchlists={watchlists} membershipIds={selectedMembershipIds} busy={busy || bulkBusy} onSave={patch => onSaveAccount(selectedRow.system_id, patch)} onToggleMembership={(watchlistId, enabled) => onToggleMembership(selectedRow.system_id, watchlistId, enabled)} />
          <button className="workflow-open-full" onClick={() => onOpenAccount(selectedRow)}>Open full account evidence →</button>
        </> : <div className="workflow-inspector-empty"><span className="page-kicker">Account inspector</span><strong>Select any account, change or action</strong><p>The inspector keeps private status, next action, note and watchlist controls beside the portfolio so you do not have to open every account profile to manage workflow.</p></div>}
      </aside>
    </div>

    <details className="workflow-command-toolbox">
      <summary><div><strong>Saved views & watchlists</strong><span>{savedViews.length} saved prospect view{savedViews.length === 1 ? '' : 's'} · {watchlists.length} watchlist{watchlists.length === 1 ? '' : 's'}</span></div><span>Expand</span></summary>
      <div className="workflow-command-toolbox-grid"><section><span className="page-kicker">Saved prospect views</span>{savedViews.length ? <div className="workflow-toolbox-list">{savedViews.slice(0, 20).map(view => <span key={view.id}>▤ {view.name}</span>)}</div> : <p>No saved prospect views yet.</p>}</section><section><span className="page-kicker">Private watchlists</span>{watchlists.length ? <div className="workflow-toolbox-list">{watchlists.map(watchlist => <button key={watchlist.id} onClick={() => { setWatchlistFilter(watchlist.id); setTab('accounts') }}>★ {watchlist.name}<strong>{watchlistCounts.get(watchlist.id) ?? 0}</strong></button>)}</div> : <p>No private watchlists yet.</p>}</section></div>
    </details>
  </section>
}
