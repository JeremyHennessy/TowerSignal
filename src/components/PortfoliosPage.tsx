...
function ownerGroups(rows: SystemSummary[]): OwnerGroup[] {
  ...
  rows.forEach(row => {
    ...
    const current = groups.get(key) ?? { key, name, rows: [], towers: 0, buildingAreaSqft: 0, highPriority: 0, contactReady: 0 }
...
export function PortfoliosPage({ payload, watchedSystemIds, onOpenAccount }: { payload: SystemsPayload; watchedSystemIds: Set<string>; onOpenAccount: (row: SystemSummary) => void }) {
  ...
  return <section className="product-page portfolios-page">
    ...
    {groups.length === 0 ? <div className="portfolio-readiness-layout">
      ...
      <div className="portfolio-evidence-card"><span className="page-kicker">Portfolio evidence rule</span><h3>Grouping is context, not corporate-parent proof.</h3><p>Build 015 groups only identical PLUTO owner names after whitespace/case normalization. HPD registration/contact and ACRIS party evidence should strengthen future confidence; similar names alone are not enough.</p><div className="evidence-pill-row"><span>PLUTO · exact BBL</span><span>HPD · exact registration</span><span>ACRIS · exact document</span></div></div>
    </div> : <div className="portfolio-layout">
      <aside className="portfolio-list"><div className="portfolio-list-heading"><span className="page-kicker">Ownership groups</span><strong>{groups.length} multi-property {groups.length === 1 ? 'group' : 'groups'}</strong></div>{groups.slice(0, 20).map(group => <article key={group.key} className={selectedGroup?.key === group.key ? 'active' : ''}><button className="portfolio-group-button" onClick={() => setSelectedKey(group.key)}><div><strong>{group.name}</strong><span>PLUTO owner-name context</span></div><dl><div><dt>Accounts</dt><dd>{group.rows.length}</dd></div><div><dt>Towers</dt><dd>{group.towers}</dd></div><div><dt>High priority</dt><dd>{group.highPriority}</dd></div></dl></button></article>)}</aside>
