...
type TorontoWorkspace = 'market' | 'monitor' | 'prospects' | 'opportunities' | 'companies' | 'portfolios' | 'watchlist' | 'sources'
...
export function TorontoParityShell({ explorer }: { explorer: ReactNode }) {
  ...
  useEffect(() => {
    if (view === 'market' || view === 'monitor' || payload || error) return
  ...
  return <div className="toronto-parity-shell">
    ...
    {view === 'market' ? explorer : <>
      ...
      {view !== 'monitor' && error && <section className="product-page toronto-page"><div className="reference-empty-state"><strong>Toronto commercial workspace unavailable.</strong><span>{error}</span></div></section>}
      {view !== 'monitor' && !error && !payload && <section className="product-page toronto-page"><div className="portal-loading">Loading Toronto commercial intelligence…</div></section>}
