...
import { TorontoMonitorPage } from './TorontoMonitorPage'
...
export function TorontoParityShell({ explorer }: { explorer: ReactNode }) {
  ...
  return <div className="toronto-parity-shell">
    ...
    {view === 'market' ? explorer : <>
      ...
      {error && view !== 'monitor' && view !== 'benchmarking' && <section className="product-page toronto-page"><div className="reference-empty-state"><strong>Toronto commercial workspace unavailable.</strong><span>{error}</span></div></section>}
      {view !== 'monitor' && view !== 'benchmarking' && !error && !payload && <section className="product-page toronto-page"><div className="portal-loading">Loading Toronto commercial intelligence…</div></section>}
      ...
      {payload && view === 'watchlist' && <section className="product-page toronto-page toronto-parity-page">
        <div className="product-page-heading"><div><span className="page-kicker">Toronto · browser-local workflow</span><h1>Watchlist</h1><p>Save Toronto properties for repeat review without enabling the shared NY workflow backend. This beta watchlist stays in this browser. Monitor separately shows verified release-to-release source changes.</p></div></div>
        {watchRows.length ? <><div className="toronto-parity-toolbar toronto-watchlist-toolbar"><div className="toronto-parity-toolbar-actions"><button onClick={() => exportProspects(watchRows, 'towersignal-toronto-watchlist.csv')}>Export watchlist CSV</button>{copyFeedback && <small role="status">{copyFeedback}</small>}</div><strong>{watchRows.length.toLocaleString()} watched properties</strong></div><ProspectTable rows={watchRows} watchedIds={watchedIds} onToggleWatch={toggleWatch} onOpen={openProperty} onCopyLead={copyLead} /></> : <div className="reference-empty-state"><strong>No Toronto properties are watched yet.</strong><span>Add properties from Prospects or Opportunities.</span></div>}
      </section>}
