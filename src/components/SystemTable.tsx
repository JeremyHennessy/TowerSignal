...
type SortKey = 'priority_score' | 'address' | 'active_equipment' | 'days_since_latest_sample' | 'latest_inspection_date' | 'oath_case_count' | 'latest_acris_recorded_date'
...
export function SystemTable({ rows, onSelect }: { rows: SystemSummary[]; onSelect: (row: SystemSummary) => void }) {
  ...
  const sorted = useMemo(() => [...rows].sort((a,b) => {
    const av = (a as EnrichedSystemSummary)[sort.key] ?? ''; const bv = (b as EnrichedSystemSummary)[sort.key] ?? ''
  ...
  const changeSort = (key: SortKey) => setSort(current => current.key === key ? { key, dir: current.dir === 'asc' ? 'desc' : 'asc' } : { key, dir: key === 'priority_score' || key === 'latest_acris_recorded_date' ? 'desc' : 'asc' })
