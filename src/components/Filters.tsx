...
export function filterSystems(rows: SystemSummary[], filters: FilterState): SystemSummary[] {
  ...
  return rows.filter((row) => {
    ...
    const enforcement = row as SystemSummary & PropertyEnforcementSummaryFields
    ...
    if (filters.hpdOpenViolations === 'true' && (enforcement.hpd_open_violation_count ?? 0) < 1) return false
    if (filters.hpdOpenViolations === 'false' && (enforcement.hpd_open_violation_count ?? 0) > 0) return false
    if (filters.stopWorkOrders === 'true' && (enforcement.stop_work_order_event_count ?? 0) < 1) return false
    if (filters.stopWorkOrders === 'false' && (enforcement.stop_work_order_event_count ?? 0) > 0) return false
    if (filters.facadeStatus && enforcement.facade_latest_status !== filters.facadeStatus) return false
...
const labels: Partial<Record<keyof FilterState, string>> = {
  borough: 'Borough', zip: 'ZIP', signal: 'Signal', confirmed: 'Violation', violationType: 'Violation type', oath: 'OATH', hpdContacts: 'Contacts', acrisActivity: 'ACRIS activity',
  hpdOpenViolations: 'HPD open violations', stopWorkOrders: 'SWO evidence', facadeStatus: 'FISP status',
  minSampleDays: 'Sample age', minEquipment: 'Equipment', minScore: 'Min score', maxScore: 'Max score', search: 'Search',
}
...
export function Filters({ rows, value, onChange, onQuick, acrisAvailable = false }: { rows: SystemSummary[]; value: FilterState; onChange: (next: FilterState) => void; onQuick: (kind: string) => void; acrisAvailable?: boolean }) {
  ...
  return <section className="filters filter-panel" aria-label="Lead filters">
    ...
    <div className="filter-grid filter-stack">
      ...
      <label>HPD open violations<select aria-label="HPD open violations" value={value.hpdOpenViolations} onChange={e => set('hpdOpenViolations', e.target.value)}><option value="">Either</option><option value="true">Open violation present</option><option value="false">No open violation matched</option></select></label>
      <label>SWO complaint/disposition evidence<select aria-label="SWO complaint/disposition evidence" value={value.stopWorkOrders} onChange={e => set('stopWorkOrders', e.target.value)}><option value="">Either</option><option value="true">SWO disposition evidence present</option><option value="false">No SWO disposition evidence matched</option></select></label>
      <label>FISP / Local Law 11 status<select aria-label="FISP / Local Law 11 status" value={value.facadeStatus} onChange={e => set('facadeStatus', e.target.value)}><option value="">Any published status</option>{facadeStatuses.map(status => <option key={status} value={status}>{status}</option>)}</select></label>
