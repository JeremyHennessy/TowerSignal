...
export interface FilterState {
  search: string; borough: string; zip: string; signal: string; confirmed: string; violationType: string;
  minSampleDays: string; minEquipment: string; minScore: string; maxScore: string
}

export const initialFilters: FilterState = { search:'', borough:'', zip:'', signal:'', confirmed:'', violationType:'', minSampleDays:'', minEquipment:'', minScore:'', maxScore:'' }
...
export function Filters({ rows, value, onChange, onQuick }: { rows: SystemSummary[]; value: FilterState; onChange: (next: FilterState) => void; onQuick: (kind: string) => void }) {
  ...
  return <section className="filters" aria-label="Lead filters">
    <div className="quick-filters" aria-label="Quick filters">
      {['Confirmed violations','Sampling-gap signals','No sample date','3+ active units','Manhattan','Highest priority'].map(label => <button key={label} className="quick" onClick={() => onQuick(label)}>{label}</button>)}
