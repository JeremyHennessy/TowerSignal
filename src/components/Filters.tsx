...
export interface FilterState {
  search: string; borough: string; zip: string; signal: string; confirmed: string; violationType: string; oath: string; hpdContacts: string;
  minSampleDays: string; minEquipment: string; minScore: string; maxScore: string
}

export const initialFilters: FilterState = { search:'', borough:'', zip:'', signal:'', confirmed:'', violationType:'', oath:'', hpdContacts:'', minSampleDays:'', minEquipment:'', minScore:'', maxScore:'' }
...
const labels: Partial<Record<keyof FilterState, string>> = {
  borough: 'Borough', zip: 'ZIP', signal: 'Signal', confirmed: 'Violation', violationType: 'Violation type', oath: 'OATH', hpdContacts: 'Contacts', minSampleDays: 'Sample age', minEquipment: 'Equipment', minScore: 'Min score', maxScore: 'Max score', search: 'Search',
}
...
export function Filters({ rows, value, onChange, onQuick }: { rows: SystemSummary[]; value: FilterState; onChange: (next: FilterState) => void; onQuick: (kind: string) => void }) {
  ...
  return <section className="filters filter-panel" aria-label="Lead filters">
    ...
    <div className="quick-filters" aria-label="Quick filters">
      {['Highest priority','Sampling-gap signals','OATH cases','Confirmed violations','No sample date','3+ active units','Manhattan'].map(label => <button key={label} className="quick" onClick={() => onQuick(label)}>{label}</button>)}
    </div>
