...
export function Filters({ rows, value, onChange, onQuick, acrisAvailable }: { rows: SystemSummary[]; value: FilterState; onChange: (next: FilterState) => void; onQuick: (kind: string) => void; acrisAvailable: boolean }) {
  ...
  const quickFilters = ['Confirmed violations','OATH cases',...(acrisAvailable ? ['Recent ACRIS activity'] : []),'Sampling-gap signals','No sample date','3+ active units','Manhattan','Highest priority']
  return <section className="filters" aria-label="Lead filters">
    ...
    <div className="filter-grid">
      ...
      <label>HPD registered contacts<select value={value.hpdContacts} onChange={e => set('hpdContacts', e.target.value)}><option value="">Either</option><option value="true">Contact records present</option><option value="false">No contact records matched</option></select></label>
      <label>ACRIS recorded activity<select value={value.acrisActivity} onChange={e => set('acrisActivity', e.target.value)} disabled={!acrisAvailable}><option value="">{acrisAvailable ? 'Any' : 'Verified cache unavailable'}</option>{acrisAvailable && <option value="true">Recent exact-BBL activity</option>}</select></label>
