...
function exportTorontoCsv(rows: TorontoProperty[]) {
  const columns = ['property_id', 'address_point_id', 'display_address', 'municipality', 'tower_evidence_status', 'source_keys', 'relationship_count', 'latitude', 'longitude']
  const escape = (value: unknown) => `"${String(value ?? '').replaceAll('"', '""')}"`
  const body = rows.map(row => [row.property_id, row.address_point_id, row.display_address, row.municipality, row.tower_evidence_status, row.source_keys.join('|'), row.relationships.length, row.latitude, row.longitude].map(escape).join(','))
...
function PropertyDetail({ property, onClose }: { property: TorontoProperty; onClose: () => void }) {
  return <aside className="toronto-detail" aria-label="Toronto property detail">
    ...
    <section className="toronto-detail-section"><h3>Source-backed property links <span>{property.source_links.length}</span></h3>{property.source_links.length ? <div className="toronto-source-list">{property.source_links.map(link => <article key={`${link.source_key}:${link.source_record_id}`}><strong>{link.source_key.replaceAll('_', ' ')}</strong><span>{link.source_record_id}</span><small>{link.match_basis.replaceAll('_', ' ').toLowerCase()}</small></article>)}</div> : <p>No joined enrichment record beyond the municipal property spine.</p>}</section>
    <section className="toronto-detail-section"><h3>Organizations and roles <span>{property.relationships.length}</span></h3>{property.relationships.length ? <div className="toronto-source-list">{property.relationships.map((relationship, index) => <article key={`${relationship.relationship}:${relationship.organization}:${index}`}><strong>{relationship.organization}</strong><span>{relationship.relationship.replaceAll('_', ' ')}</span><small>{relationship.source_key.replaceAll('_', ' ')} · {relationship.confidence.toLowerCase()}</small></article>)}</div> : <p>No defensible organization relationship is currently attached.</p>}</section>
  </aside>
}
...
export function TorontoMarketPage() {
  ...
  const filtered = useMemo(() => {
    ...
    return payload.properties.filter(property => {
      ...
      if (relationshipOnly && property.relationships.length === 0) return false
      return true
    })
  }, [payload, search, evidence, source, relationshipOnly])
  ...
  return <section className="product-page toronto-page">
    ...
    <div className="toronto-filters">
      ...
      <label><span>Source family</span><select value={source} onChange={event => setSource(event.target.value)}><option value="">All source families</option>{sources.map(value => <option key={value} value={value}>{value.replaceAll('_', ' ')}</option>)}</select></label>
      <label className="toronto-check"><input type="checkbox" checked={relationshipOnly} onChange={event => setRelationshipOnly(event.target.checked)} /><span>Has organization relationship</span></label>
