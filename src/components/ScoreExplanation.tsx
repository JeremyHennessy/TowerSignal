...
type ReviewedComponent = SystemDetail['scoring']['components'][number] & { source?: string; source_date?: string | null }
type ReviewedScoring = Omit<SystemDetail['scoring'], 'components'> & { components: ReviewedComponent[]; as_of?: string; notes?: string[]; context_note?: string; uncapped_score?: number }
export function ScoreExplanation({ scoring }: { scoring: ReviewedScoring }) {
  return <section className="reviewed-score" aria-label="Priority score explanation"><h3>Why this score</h3>
    {scoring.components.length ? <ul className="score-list">{scoring.components.map((component, index) => <li key={`${component.reason}-${index}`}><strong>+{component.points}</strong><span>{component.reason}{component.source && <small className="score-component-source">{component.source.startsWith('https://www.nyc.gov/') ? <a href={component.source} target="_blank" rel="noreferrer">Official source ↗</a> : component.source}{component.source_date ? ` · ${formatDate(component.source_date)}` : ''}</small>}</span></li>)}</ul> : <div className="empty-inline">No priority points were assigned. This does not certify safety or compliance.</div>}
