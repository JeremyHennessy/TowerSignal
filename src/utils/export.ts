...
export function exportCsv(rows: SystemSummary[], metadata: Metadata): void {
  const headers = ['address','borough','zip','system_id','bin','active_equipment','latest_public_sample_date','days_since_sample','signal_type','confirmed_violation','latest_inspection_date','priority_score','evidence_confidence','source_snapshot_timestamp']
  ...
  for (const row of rows) {
    lines.push([
      ...
      signalLabel(row.primary_signal),row.confirmed_violation,row.latest_inspection_date,row.priority_score,row.evidence_confidence,metadata.generated_at,
    ].map(csvCell).join(','))
  }
