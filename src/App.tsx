...
export default function App() {
  ...
  return <main className="app-shell">
    ...
    <footer><div><strong>Source snapshot</strong>{payload.metadata.sources.map(source => <span key={source.dataset_id}>{source.name} · {source.dataset_id} · {source.source_record_count.toLocaleString()} {source.source_query_scope ? 'rows returned by scoped query' : 'source rows'}{source.matched_record_count != null ? ` · ${source.matched_record_count.toLocaleString()} exact-matched cases` : ''}</span>)}</div><div>Rules {payload.metadata.rules_version} · Priority model {payload.metadata.priority_model_version}</div></footer>
