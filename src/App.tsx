  ...
  return <main className="app-shell saas-shell">
    ...
    <div className="main-stage">
      <header className="utility-bar">
        ...
        <div className="utility-actions"><span className="coverage-chip">Data refreshed {formatTimestamp(nysMode ? nysPayload.metadata.generated_at : payload.metadata.generated_at)}</span>{!nysMode && <button className="primary" onClick={() => exportCsv(filtered, payload.metadata)}>Export {filtered.length.toLocaleString()} accounts</button>}</div>
      </header>

      {mode === 'prospect' && <>
        <section className="value-hero">
          <div><span className="eyebrow">Commercial timing intelligence</span><h1>Know which cooling-tower accounts deserve attention now.</h1><p>TowerSignal combines public registration, sampling, inspection, OATH, DOB, property and contact evidence into a prioritized workspace for water-treatment, Legionella, HVAC and environmental sales teams.</p></div>
        ...
        <section className="signal-grid" aria-label="Commercial signal summary">
          ...
          <article><span className="metric-icon">⌁</span><div><small>Recent DOB activity</small><strong>{recentDob.toLocaleString()}</strong><span>Commercial timing context</span></div></article>
        </section>
