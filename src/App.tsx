...
export default function App() {
  ...
  return <main className="app-shell">
    <header className="topbar"><div><div className="brand"><span className="brand-mark">TS</span><div><h1>TowerSignal</h1><p>{nysMode ? 'NYS cooling-tower registry intelligence' : 'NYC cooling-tower commercial intelligence'}</p></div></div></div><div className="freshness"><span>Data refreshed</span><strong>{formatTimestamp(nysMode ? nysPayload.metadata.generated_at : payload.metadata.generated_at)}</strong>{sourceHealth.length > 0 && <span>Source health {healthyHealth.length}/{sourceHealth.length} healthy</span>}{failedHealth.length > 0 && <em>{failedHealth.length} source-health failure{failedHealth.length === 1 ? '' : 's'}</em>}{warningHealth.length > 0 && failedHealth.length === 0 && <em>{warningHealth.length} source-health warning{warningHealth.length === 1 ? '' : 's'}</em>}{generatedAgeDays > 2 && <em>Snapshot is older than expected</em>}{oldestSourceUpdateDays > 30 && <em>One NYC source was last updated {oldestSourceUpdateDays} days ago</em>}</div></header>
