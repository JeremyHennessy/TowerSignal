...
type ProductMode = 'leads' | 'changes' | 'nys' | 'nys-changes'
...
export default function App() {
  ...
  if (!payload || !changes || !nysPayload || !nysChanges) return <main className="app-shell"><div className="loading-page"><div className="brand-mark">TS</div><h1>TowerSignal</h1><p>Loading current public-record snapshots and preserved history…</p></div></main>
  ...
  return <main className="app-shell">
    <header className="topbar"><div><div className="brand"><span className="brand-mark">TS</span><div><h1>TowerSignal</h1><p>{nysMode ? 'NYS cooling-tower registry intelligence' : 'NYC cooling-tower commercial intelligence'}</p></div></div></div><div className="freshness"><span>Data refreshed</span><strong>{formatTimestamp(nysMode ? nysPayload.metadata.generated_at : payload.metadata.generated_at)}</strong>{sourceHealth.length > 0 && <span>Source health {healthyHealth.length}/{sourceHealth.length} healthy</span>}{!nysMode && acrisAvailable && acrisMetadata.acris_cache_generated_at && <span>ACRIS cache {formatTimestamp(acrisMetadata.acris_cache_generated_at)}</span>}{!nysMode && !acrisAvailable && <em>ACRIS timing cache unavailable</em>}{failedHealth.length > 0 && <em>{failedHealth.length} source-health failure{failedHealth.length === 1 ? '' : 's'}</em>}{warningHealth.length > 0 && failedHealth.length === 0 && <em>{warningHealth.length} source-health warning{warningHealth.length === 1 ? '' : 's'}</em>}{generatedAgeDays > 2 && <em>Snapshot is older than expected</em>}{oldestSourceUpdateDays > 30 && <em>One NYC source was last updated {oldestSourceUpdateDays} days ago</em>}</div></header>
    <nav className="product-nav" aria-label="TowerSignal product modes"><button className={mode === 'leads' ? 'active' : ''} onClick={() => setMode('leads')}>Leads</button><button className={mode === 'changes' ? 'active' : ''} onClick={() => setMode('changes')}>Changes</button><button className={mode === 'nys' ? 'active' : ''} onClick={() => setMode('nys')}>NYS Registry</button><button className={mode === 'nys-changes' ? 'active' : ''} onClick={() => setMode('nys-changes')}>NYS Changes</button></nav>
    <div hidden={mode !== 'leads'}>
      <section className="hero"><div><span className="eyebrow">Source-backed commercial intelligence</span><h2>Find cooling-tower systems worth investigating today.</h2><p>Turn public registration, sampling, NYC Health inspection, OATH lifecycle, DOB project, HPD contact and verified ACRIS property-timing records into transparent commercial intelligence — with evidence and uncertainty kept separate.</p></div><div className="hero-actions"><button className="primary" onClick={() => exportCsv(filtered, payload.metadata)}>Export filtered CSV</button><span>{filtered.length.toLocaleString()} records in current lead set</span></div></section>
    ...
    {mode === 'changes' && <ChangesView payload={changes} onSelectSystem={selectById} />}
    {mode === 'nys' && <NysRegistryView payload={nysPayload} selected={selectedNys} onSelect={setSelectedNys} />}
    {mode === 'nys-changes' && <NysChangesView payload={nysChanges} systems={nysPayload.systems} onSelect={setSelectedNys} />}
    <footer><div><strong>Source health</strong>{sourceHealth.length === 0 ? <span>Source-health metrics unavailable for this snapshot.</span> : sourceHealth.map(source => <span key={source.source_key}>{source.name} · {source.status} · {source.coverage_percentage == null ? 'coverage n/a' : `${source.coverage_percentage.toFixed(1)}% coverage`} · {source.attached_entity_count.toLocaleString()} attached · {source.displayed_entity_count.toLocaleString()} represented</span>)}</div><div><strong>Source snapshot</strong>{payload.metadata.sources.map(source => <span key={source.dataset_id}>{source.name} · {source.dataset_id} · {source.source_record_count.toLocaleString()} {source.source_query_scope ? 'rows returned by scoped query' : 'source rows'}{source.matched_record_count != null ? ` · ${source.matched_record_count.toLocaleString()} exact-matched cases` : ''}</span>)}<span>{nysPayload.metadata.source.name} · {nysPayload.metadata.source.dataset_id} · {nysPayload.metadata.source.source_record_count.toLocaleString()} source rows</span></div><div>Rules {payload.metadata.rules_version} · Priority model {payload.metadata.priority_model_version} · NYC history {changes.history_schema_version} · NYS history {nysChanges.history_schema_version}</div></footer>
    {(mode === 'leads' || mode === 'changes') && <DetailPanel row={selected} metadata={payload.metadata} historyEvents={selected ? changes.events.filter(event => event.system_id === selected.system_id) : []} historyStartedAt={changes.history_started_at} onClose={() => setSelected(null)} />}
