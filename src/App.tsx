...
export default function App() {
  ...
  return <main className="app-shell">
    ...
    <section className="hero"><div><span className="eyebrow">Source-backed commercial intelligence</span><h2>Find cooling-tower systems worth investigating today.</h2><p>Turn public registration, sampling and NYC Health inspection records into transparent commercial signals — with evidence and uncertainty kept separate.</p></div><div className="hero-actions"><button className="primary" onClick={() => exportCsv(filtered, payload.metadata)}>Export filtered CSV</button><span>{filtered.length.toLocaleString()} records in current lead set</span></div></section>
    ...
    <footer><div><strong>Source snapshot</strong>{payload.metadata.sources.map(source => <span key={source.dataset_id}>{source.name} · {source.dataset_id} · {source.source_record_count.toLocaleString()} rows</span>)}</div><div>Rules {payload.metadata.rules_version} · Priority model {payload.metadata.priority_model_version}</div></footer>
