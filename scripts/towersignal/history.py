...
def _event(event_type: str, current: dict[str, Any], detected_at: str, source: str, evidence_basis: str,
           previous_value: Any, new_value: Any, source_observation_date: str | None = None) -> dict[str, Any]:
    return {
        "event_type": event_type,
        "system_id": current["system_id"], "bbl": current.get("bbl"), "bin": current.get("bin"),
        "address": current.get("address"), "borough": current.get("borough"),
        "detected_at": detected_at, "source_observation_date": source_observation_date,
        "previous_value": previous_value, "new_value": new_value, "source": source,
        "evidence_basis": evidence_basis, "priority_score": current.get("priority_score"),
...
def _compact_inspection_identity(item: dict[str, Any]) -> dict[str, Any]:
    return {"inspection_date": item.get("inspection_date"), "inspection_type": item.get("inspection_type"), "status": item.get("status")}


def _compact_violation_identity(item: dict[str, Any]) -> dict[str, Any]:
    identity = {"summons_number": item.get("summons_number"), "violation_code": item.get("violation_code"),
                "law_section": item.get("law_section"), "violation_type": item.get("violation_type")}
...
def _compact_violation_event(item: dict[str, Any]) -> dict[str, Any]:
    return {"summons_number": item.get("summons_number"), "violation_code": item.get("violation_code"),
            "law_section": item.get("law_section"), "violation_type": item.get("violation_type"),
...
def _compact_oath_case(item: dict[str, Any]) -> dict[str, Any]:
    return {key: item.get(key) for key in (
        "ticket_number", "hearing_status", "hearing_result", "hearing_date", "decision_date",
        "violation_date", "penalty_imposed", "paid_amount", "balance_due"
    )}
...
def _compact_building_context(item: dict[str, Any] | None) -> dict[str, Any] | None:
    if not item:
        return None
    return {key: item.get(key) for key in (
        "owner_name", "land_use", "building_class", "year_built", "building_area_sqft", "floors", "total_units"
    )}


def _compact_contact(item: dict[str, Any]) -> dict[str, Any]:
    return {
        "registration_contact_id": item.get("registration_contact_id"), "type": item.get("type"),
        "description": _bounded(item.get("description")), "corporation_name": item.get("corporation_name"),
        "person_name": item.get("person_name"), "title": item.get("title"),
...
def build_observation(system: dict[str, Any], summary_row: dict[str, Any], inspections: list[dict[str, Any]],
    ...
    for inspection in inspections:
        identity = _compact_inspection_identity(inspection)
        inspection_records[_fingerprint(identity)] = identity
        for violation in inspection.get("violations") or []:
            violation_identity = _compact_violation_identity(violation)
            violation_records[_fingerprint(violation_identity)] = {
                **_compact_violation_event(violation), "inspection_date": inspection.get("inspection_date")
            }
    ...
    return {
        ...
        "primary_signal": summary_row.get("primary_signal"), "signal_types": summary_row.get("signal_types", []),
        "priority_score": summary_row.get("priority_score"), "evidence_confidence": summary_row.get("evidence_confidence"),
        "inspection_keys": sorted(inspection_records), "violation_keys": sorted(violation_records),
...
def _contact_key(item: dict[str, Any]) -> str:
    return _fingerprint({key: item.get(key) for key in (
        "registration_contact_id", "type", "corporation_name", "person_name", "title", "business_address"
    )})
...
def detect_changes(previous: dict[str, Any], current: dict[str, Any], detected_at: str) -> list[dict[str, Any]]:
    ...
    previous_samples, current_samples = set(previous.get("sample_dates") or []), set(current.get("sample_dates") or [])
    ...
    previous_signals, current_signals = set(previous.get("signal_types") or []), set(current.get("signal_types") or [])
    ...
    for key in sorted(set(current.get("inspection_keys") or []) - set(previous.get("inspection_keys") or [])):
    ...
    for key in sorted(set(current.get("violation_keys") or []) - set(previous.get("violation_keys") or [])):
    ...
    previous_oath, current_oath = _oath_map(previous.get("oath_cases") or []), _oath_map(current.get("oath_cases") or [])
...
def build_history(current_observations: list[dict[str, Any]], detected_at: str, previous_snapshot: dict[str, Any] | None,
