...
def validate_normalized(systems: list[dict[str, Any]], snapshot_date: date) -> None:
    ...
    for system in systems:
        ...
        if lat is not None and not 40.3 <= lat <= 41.1:
            raise DataValidationError(f"Implausible NYC latitude for system {system['system_id']}: {lat}")
        if lon is not None and not -74.4 <= lon <= -73.5:
            raise DataValidationError(f"Implausible NYC longitude for system {system['system_id']}: {lon}")
