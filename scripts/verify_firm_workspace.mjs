...
for (const family of ['desktop', 'iphone']) {
  ...
  try {
    ...
    const labNames = new Set(payload.firms.filter(row => row.roles.includes('DWT_LABORATORY')).map(row => row.canonical_name))
    await expect.poll(async () => {
      const names = await table.locator('tbody tr td:first-child strong').allTextContents()
      return names.length > 0 && names.every(name => labNames.has(name))
    }).toBe(true)
    ...
    await expect(table.getByRole('button', { name: 'Company / firm ↑', exact: true })).toBeVisible()
    const names = await table.locator('tbody tr td:first-child strong').allTextContents()
    assert(names.length > 1 && names[0].localeCompare(names[1], undefined, { numeric: true, sensitivity: 'base' }) <= 0, 'Company sorting failed')
    const firm = payload.firms.find(row => row.canonical_name === 'Nalco Company' && row.mapped_site_count > 0)
      ?? payload.firms.find(row => row.mapped_site_count > 0 && row.tower_account_count > 0 && row.observed_site_count > 50 && row.observed_site_count < 500)
