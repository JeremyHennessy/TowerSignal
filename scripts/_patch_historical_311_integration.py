...
replace_once(
    ...
    "      - name: Attach exact-BBL/BIN NYC building-water signals\n        run: python scripts/attach_nyc_water_signals.py --output public/data --cache public/data/nyc-water-signals.json\n      - name: Build exact-count NYC lead service-line cache\n",
    "      - name: Attach exact-BBL/BIN NYC building-water signals\n        run: python scripts/attach_nyc_water_signals.py --output public/data --cache public/data/nyc-water-signals.json\n      - name: Build compact exact-BBL historical NYC 311 context\n        timeout-minutes: 15\n        run: python scripts/build_historical_311_context.py --systems public/data/systems.json --output public/data/historical-311-context.json --bbl-batch-size 200 --page-size 5000\n      - name: Validate historical NYC 311 context\n        run: python scripts/validate_historical_311_context.py --cache public/data/historical-311-context.json --require-production-volume\n      - name: Attach compact historical NYC 311 context\n        run: python scripts/attach_historical_311_context.py --output public/data --cache public/data/historical-311-context.json\n      - name: Build exact-count NYC lead service-line cache\n",
)
