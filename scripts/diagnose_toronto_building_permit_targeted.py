...
SQL_ENDPOINT = "https://ckan0.cf.opendata.inter.prod-toronto.ca/api/3/action/datastore_search_sql"
...
def sql(query: str) -> dict[str, Any]:
    payload = request_json(f"{SQL_ENDPOINT}?{urlencode({'sql': query})}", timeout=180)
    if payload.get("success") is not True:
        raise RuntimeError(f"Toronto CKAN SQL failed: {query[:300]}")
    result = payload.get("result") or {}
    if not isinstance(result, dict):
        raise RuntimeError("Unexpected Toronto CKAN SQL result")
    return result


def where_for(term: str) -> str:
    escaped = term.replace("'", "''").lower()
    return f"LOWER(COALESCE(\"DESCRIPTION\", '')) LIKE '%{escaped}%'"


def count_query(resource_id: str, where: str) -> int:
    result = sql(f'SELECT COUNT(*) AS count FROM "{resource_id}" WHERE {where}')
    rows = result.get("records") or []
    return int(rows[0].get("count") or 0) if rows else 0


def fetch_rows(resource_id: str, where: str, total: int) -> list[dict[str, Any]]:
    rows: list[dict[str, Any]] = []
    offset = 0
    while offset < total:
        result = sql(f'SELECT * FROM "{resource_id}" WHERE {where} ORDER BY "_id" LIMIT 5000 OFFSET {offset}')
        ...
        offset += len(batch)
    if len(rows) != total:
        raise RuntimeError(f"Targeted permit pagination mismatch: expected {total}, found {len(rows)}")
    return rows
...
def summarize(source_name: str, resource_id: str, by_apid: dict[str, str], by_address: dict[str, list[str]]) -> tuple[dict[str, Any], set[str]]:
    term_counts = {key: count_query(resource_id, where_for(term)) for key, term in SIGNALS.items()}
    combined_where = " OR ".join(f"({where_for(term)})" for term in SIGNALS.values())
    total = count_query(resource_id, combined_where)
    rows = fetch_rows(resource_id, combined_where, total)
    ...
    for row in rows:
        ...
        if identity in identities:
            raise RuntimeError(f"Duplicate targeted permit identity in {source_name}: {identity}")
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Read-only targeted Toronto mechanical building-permit diagnostic")
    parser.add_argument("--output", type=Path, default=Path("toronto-building-permit-targeted.json"))
    ...
    report = {
        "schema_version": "toronto-building-permit-targeted-diagnostic-1.0",
        "status": "PASSED_DIAGNOSTIC",
        "scope": "Read-only live source diagnostic. Target terms select permit activity only; they do not promote cooling-tower confirmation or organization relationships.",
