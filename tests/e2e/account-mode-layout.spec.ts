...
test('account mode tabs span the report and switch visible evidence on desktop and iPhone', async ({ page }, testInfo) => {
  ...
  const hero = detail.locator('.account-profile-hero')
  await expect(hero).toBeVisible()
  await expect(detail.locator('.detail-header')).toHaveCount(0)
  await expect(detail.locator('.detail-actions')).toHaveCount(0)
  await expect(hero.getByRole('heading', { name: '16 E 39TH ST', level: 1 })).toBeVisible()
  await expect(hero.getByRole('button', { name: 'Copy lead brief', exact: true })).toBeVisible()
  await expect(hero.getByRole('button', { name: 'Copy account link', exact: true })).toBeVisible()
  await expect(hero.getByRole('button', { name: 'Export account', exact: true })).toBeVisible()
  const shellGeometry = await page.evaluate(() => {
    const hero = document.querySelector<HTMLElement>('.account-profile-hero')
    const tabs = document.querySelector<HTMLElement>('.account-mode-tabs')
    const detail = document.querySelector<HTMLElement>('.account-profile-page .detail-panel')
    if (!hero || !tabs || !detail) return null
    const heroBox = hero.getBoundingClientRect()
    const tabsBox = tabs.getBoundingClientRect()
    const style = getComputedStyle(detail)
    return { heroBottom: heroBox.bottom, tabsTop: tabsBox.top, overflowY: style.overflowY, maxHeight: style.maxHeight }
  })
  expect(shellGeometry).not.toBeNull()
  expect(shellGeometry!.heroBottom).toBeLessThanOrEqual(shellGeometry!.tabsTop + 2)
  expect(shellGeometry!.overflowY).toBe('visible')
  ...
  const historyOrder = await detail.evaluate(panel => [...panel.children].map((element, domIndex) => {
    if (!(element instanceof HTMLElement) || element.hidden) return null
    const heading = element.querySelector('h3')?.textContent?.trim() ?? ''
    const cssOrder = Number.parseInt(getComputedStyle(element).order || '0', 10) || 0
    return { domIndex, cssOrder, heading, timeline: element.classList.contains('account-unified-timeline') }
  }).filter(Boolean))
  const timelineOrder = historyOrder.find(item => item?.timeline)
  const acrisOrder = historyOrder.find(item => item?.heading === 'ACRIS property activity')
  expect(timelineOrder).toBeTruthy()
  if (acrisOrder) {
    expect([timelineOrder!.cssOrder, timelineOrder!.domIndex] < [acrisOrder.cssOrder, acrisOrder.domIndex]).toBe(false)
    const timelineBeforeAcris = timelineOrder!.cssOrder < acrisOrder.cssOrder
      || (timelineOrder!.cssOrder === acrisOrder.cssOrder && timelineOrder!.domIndex < acrisOrder.domIndex)
    expect(timelineBeforeAcris).toBe(true)
  }
