...
test('hosted production workflow survives a fresh signed-in browser session', async ({ browser }) => {
  ...
  const save1 = page1.getByRole('button', { name: 'Save workflow state', exact: true })
  await save1.click()
  await expect(page1.getByRole('button', { name: 'Saving…', exact: true })).toBeVisible({ timeout: 5_000 })
  await expect(page1.getByRole('button', { name: 'Save workflow state', exact: true })).toBeEnabled({ timeout: 15_000 })
  ...
  const save2 = page2.getByRole('button', { name: 'Save workflow state', exact: true })
  await save2.click()
  await expect(page2.getByRole('button', { name: 'Saving…', exact: true })).toBeVisible({ timeout: 5_000 })
  await expect(page2.getByRole('button', { name: 'Save workflow state', exact: true })).toBeEnabled({ timeout: 15_000 })
