...
test('Evidence mode presents seven collapsed source groups with explicit relationship boundaries', async ({ page }, testInfo) => {
  ...
  for (const [index, title] of expectedTitles.entries()) {
    await expect(groups.nth(index).locator('summary').first().locator('strong').first()).toHaveText(title)
  }
...
test('Evidence architecture loads representative source-backed account categories', async ({ page }, testInfo) => {
  test.skip(isIphoneProject(testInfo), 'Representative account matrix is desktop-only; iPhone containment is covered by the full Evidence scenarios')
  ...
  const systems = (payload.systems ?? []).filter(row => typeof row.system_id === 'string' && row.system_id.length > 0)
  expect(systems.length).toBeGreaterThan(0)

  const used = new Set<string>()
  const pick = (
    label: string,
    predicate: (row: (typeof systems)[number]) => boolean,
    sorter?: (a: (typeof systems)[number], b: (typeof systems)[number]) => number,
  ) => {
    const matches = systems.filter(row => predicate(row) && !used.has(row.system_id!))
    if (sorter) matches.sort(sorter)
    expect(matches.length, `Expected representative ${label} account in generated systems payload`).toBeGreaterThan(0)
    const selected = matches[0]
    used.add(selected.system_id!)
    return { label, systemId: selected.system_id! }
  }

  const matrix = [
    pick('high-priority', row => (row.priority_score ?? 0) >= 70, (a, b) => (b.priority_score ?? 0) - (a.priority_score ?? 0)),
    pick('no-contact', row => row.hpd_contact_count === 0),
    pick('CMS/institutional', row => (row.cms_institutional_facility_count ?? 0) > 0),
    pick('multi-tower', row => (row.active_equipment ?? 0) > 1, (a, b) => (b.active_equipment ?? 0) - (a.active_equipment ?? 0)),
    pick('DWT-heavy', row => (row.nyc_building_water_signal_count ?? 0) > 0,
      (a, b) => (b.nyc_building_water_signal_count ?? 0) - (a.nyc_building_water_signal_count ?? 0)),
    pick('outer-borough', row => Boolean(row.borough && row.borough.toUpperCase() !== 'MANHATTAN')),
  ]

  for (const candidate of matrix) {
    await test.step(candidate.label, async () => {
